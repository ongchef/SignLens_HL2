...
public class changesize : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        //backgroundSpriteRenderer1 = transform.Find("Background1").GetComponent<SpriteRenderer>();
    }
    ...
    void Update()
    {
        ...
        lock (threadLocker)
        {
            RecognizedText.SetText(recognizedString);
            RecognizedText.ForceMeshUpdate();
            Vector2 textSize = RecognizedText.GetRenderedValues(false);
            Vector2 padding = new Vector2(0.2f, 0.2f);
            backgroundSpriteRenderer.size = textSize +padding;
            if (recognizedString.Length != 0)
            {
                check = recognizedString.Substring(recognizedString.Length - 1);
            }

            if (check == "." | check == "?" | check == "!" | check == "。" | check == "？")//如果最後一個字是句號或者問號或驚嘆號 就更新下面的儲存欄
            {
                ErrorText.SetText(recognizedString);
                Vector2 textSize1 = ErrorText.GetRenderedValues(false);
                Vector2 padding1 = new Vector2(0.2f, 0.2f);
                backgroundSpriteRenderer1.size = textSize1 + padding1;
            }
